...
// 初始置换
static const uint8 Table_IP[64] = { 58, 50, 42, 34, 26, 18, 10, 2, 60, 52, 44,
		36, 28, 20, 12, 4, 62, 54, 46, 38, 30, 22, 14, 6, 64, 56, 48, 40, 32,
		24, 16, 8, 57, 49, 41, 33, 25, 17, 9, 1, 59, 51, 43, 35, 27, 19, 11, 3,
		61, 53, 45, 37, 29, 21, 13, 5, 63, 55, 47, 39, 31, 23, 15, 7 };

// 末置换
static const uint8 Table_InverseIP[64] = { 40, 8, 48, 16, 56, 24, 64, 32, 39, 7,
...
// 扩展置换
static const uint8 Table_E[48] = { 32, 1, 2, 3, 4, 5, 4, 5, 6, 7, 8, 9, 8, 9,
		10, 11, 12, 13, 12, 13, 14, 15, 16, 17, 16, 17, 18, 19, 20, 21, 20, 21,
		22, 23, 24, 25, 24, 25, 26, 27, 28, 29, 28, 29, 30, 31, 32, 1 };

// 密钥初始置换
static const uint8 Table_PC1[56] = { 57, 49, 41, 33, 25, 17, 9, 1, 58, 50, 42,
...
// 密钥压缩置换
static const uint8 Table_PC2[48] = { 14, 17, 11, 24, 1, 5, 3, 28, 15, 6, 21, 10,
...
static void Xor(uint8 * p_buf_1, const uint8 * p_buf_2, uint8 bytes) {
	while (bytes > 0) {
		bytes--;
...
//offset只能是1，2
//本段代码还可以优化。
static void move_left(uint8 * p_input, uint8 bit_start, uint8 bit_end,
		uint8 offset) {
	uint8 b_val = 0;
	uint8 b_tmp1 = 0;
	uint8 b_tmp2 = 0;
...
//offset只能是1，2
//本段代码在性能上还可以优化。
static void move_right(uint8 * p_input, uint8 bit_start, uint8 bit_end,
		uint8 offset) {
	uint8 b_val = 0;
	uint8 b_tmp1 = 0;
	uint8 b_tmp2 = 0;
...
//offset大于0时左移
//offset小于0时右移
static void move_bits(uint8 * p_input, uint8 bit_start, uint8 bit_end,
		char offset) {
	if (0 < offset) //左移
			{
...
static void Permutation(uint8 * p_input, uint8 * p_output, const uint8 * Table,
		uint8 bits) {
	uint8 b_val = FALSE;
	uint8 bit_index = 0;
...
static uint8 S_GetLine(uint8 * p_data_ext, uint8 bit_s) {
	return (GET_BIT(p_data_ext, bit_s + 0) << 1)
			+ GET_BIT(p_data_ext, bit_s + 5);
}

//获取从bit_s为起始的第2,3,4,5位组成列
static uint8 S_GetRow(uint8 * p_data_ext, uint8 bit_s) {
	uint8 row;
